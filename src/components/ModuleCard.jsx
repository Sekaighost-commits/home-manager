...
export default function ModuleCard({ id, icon, nom, path, subtitle, badge, variant, color }) {
  return (
    <Link
      to={path}
      className={`module-card ${variant ? `module-card--${variant}` : ''}`}
      style={color && !variant ? { '--card-accent': color } : undefined}
    >
      {badge != null && badge > 0 && (
